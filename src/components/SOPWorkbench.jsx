...
export default function SOPWorkbench({checks,setChecks,open,setOpen,recommended}) {
  ...
  const visible=useMemo(()=>sops.filter(s=>filter==='全部'||s.status===filter),[filter]);
  ...
  return <><div className="filter-row"><div>{['全部','立即使用','客户验证阶段','触发后启用'].map(x=><button key={x} className={filter===x?'active':''} onClick={()=>setFilter(x)}>{x}</button>)}</div><button className="danger-outline" onClick={allReset}>重置全部 SOP</button></div>
